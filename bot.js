$(function() {
    ...
     var h = d.getHours(); 
     ...
     var wrud=["WHAT R U DOING?","WHAT ARE YOU DOING?", "WHAT'S UP", "WHAT'S UP BUDDY","WHAT ARE YOU DOING"];
      ...
      var me =["WHO ARE YOU","WHO IS YO","WHAT ARE YOU","URINDE"];
     function isInArray(x, y) { return x.indexOf(y) > -1; }
     isInArray(me, lastmsg)==true? smsg="i'm an artificial inteligence system that can help you <br> in answering your queston and help you in your studing jurney created by Murenge Tech":
    isInArray(hello, lastmsg)==true?smsg="Hello, How are you? ":
    isInArray(wru, lastmsg)==true?smsg="I am Mulenge Ai i'm am artificial intelingence tool that helps human in deferent kind of job position":
    isInArray(wrud, lastmsg)==true?smsg="Nothing special and you? ":
    isInArray(bye, lastmsg)==true?(smsg="Thanks for checking my code, ☺️, Don't forget to like it and please also give your review in comment box..... Bye ",
    setTimeout(function(){$(".status").html("last seen today at "+time)},6000),
    setTimeout(function(){$(".status").css("margin-left","-50")},8000)):
    isInArray(th, lastmsg)==true?smsg="You're Welcome ":
    ...
    (smse=["I didn't understand, please explain it with proper spellings.", "<b align='center'>These are top result from internet </b><nav class='back'  onclick='history.back()'>&larr;</nav><nav class='' onclick='history.()'>&rarr;</nav><iframe style = 'z-index:1;overflow-x:scroll; overflow-y:scroll;' scrolling='yes' height='300px' width='100%' src='https://www.bing.com/search?q="+search+"'></iframe>"],
    smsg=smse[Math.floor(Math.random()*2)]);
    ...
    iv = setInterval(write, 100);
    }

    });
    function write(){
        div = document.getElementsByClassName("conversation");
        div.innerHTML += isInArray[x.indexOf];
        x.indexOf++;
        if(x.indexOf>= isInArray.length)
        clearInterval(iv);

    }
    function photo(){
        var emoji = document.getElementById("photo")
        if(click)
        alert(emoji+"still working on it")
    }
